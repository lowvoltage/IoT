...
// Use the ESP8266's MAC address to build an unique topic name
const String MQTT_OUT_TOPIC = "device/" + WiFi.BSSIDstr() + "/status";
...
// Initialize with some "out of range" value, so that first acutal reading always gets published
...
void publish(const char* message) {
  ...
  mqttClient.publish(MQTT_OUT_TOPIC.c_str(), message, true);
}
...
void reconnect() {
  ...
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect("ESP8266Client", MQTT_USERNAME, MQTT_PASSWORD,
                           MQTT_OUT_TOPIC.c_str(), 0, true, "N/A")) {
